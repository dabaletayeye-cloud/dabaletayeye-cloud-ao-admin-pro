...
页面里的版本切换只隐藏入口。本脚本会备份并物理移除不需要的 `src/modules/<模块>` 目录，随后重新生成路由、菜单、权限注册表。
...
恢复只补回当次移除的目录，不覆盖保留模块的后续开发。如果同名目录已经重新创建，脚本停止并提示冲突，请先移走冲突目录；不会提供强制覆盖选项。恢复后备份仍保留。
...
- 裁剪/恢复后重启前端开发服务，并在版本面板选择目标版本（恢复全部模块后可选全部版）。后端沿用原有模块清单目录发现配置。

    ...
    >
      ...
      <div>
        <div style={{ fontSize: 30, fontWeight: 800, color: 'var(--foreground)', lineHeight: 1, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.5px' }}>
          {item.prefix}{fmt(animated, item.suffix)}{item.suffix !== '%' && item.prefix !== '¥' ? item.suffix : ''}
          {item.suffix === '%' ? '%' : ''}
        </div>
...
export default function DashboardAnalyticsPage() {
  ...
  const STATS: StatItem[] = [
    { key: 'revenue',    label: '今日总收入', value: 128640, prefix: '¥', suffix: '',  change: 12.5,  icon: <DollarSignIcon size={20} />, iconBg: primaryAlpha,               iconColor: primaryHex   },
    { key: 'orders',     label: '今日订单数', value: 3842,   prefix: '',  suffix: '单', change: 8.3,   icon: <ShoppingBagIcon size={20} />, iconBg: 'rgba(16,185,129,0.12)',   iconColor: '#10b981'    },
    { key: 'conversion', label: '转化率',     value: 68,     prefix: '',  suffix: '%',  change: -2.1,  icon: <TargetIcon size={20} />,     iconBg: 'rgba(245,158,11,0.12)',   iconColor: '#f59e0b'    },
    { key: 'newUsers',   label: '新增客户',   value: 1247,   prefix: '',  suffix: '人', change: 15.8,  icon: <UserPlusIcon size={20} />,   iconBg: 'rgba(59,130,246,0.12)',   iconColor: '#3b82f6'    },
  ];
  ...
  const visitorOption = useMemo(() => ({
    ...
    xAxis: { ...xAxisBase(WEEK_DAYS), boundaryGap: false },
    yAxis: yAxisBase,
    series: [
      {
        name: '老客户', type: 'line', data: RETURNING, smooth: true, symbol: 'none',
      ...
      {
        name: '新客户', type: 'line', data: NEW_USER, smooth: true, symbol: 'none',
  ...
  }), [isManga, isDark, primaryHex, tipBg, tipBorder, tipText, axisLabel, gridLine]);
  ...
  const satOption = useMemo(() => ({
    ...
    series: [
      {
        name: '上月', type: 'line', data: SAT_LAST, smooth: true, symbol: 'none',
      ...
      {
        name: '本月', type: 'line', data: SAT_THIS, smooth: true, symbol: 'none',
  ...
  }), [isDark, tipBg, tipBorder, tipText, axisLabel, gridLine]);
  ...
  const targetOption = useMemo(() => ({
    ...
    series: [
      {
        name: '实际销售', type: 'bar', data: TARGET_ACT, barMaxWidth: 18,
        itemStyle: {
          borderRadius: barRadius,
          color: {
  ...
  }), [isManga, isDark, tipBg, tipBorder, tipText, axisLabel, gridLine]);

  // ── Local states ───────────────────────────────────────────────────────────
  const [exportHover, setExportHover] = useState(false);
  const handleExport = () => toast.success('分析报表已导出');
  ...
  return (
    <AdminLayout>
      ...
      >
        {/* ── 页头 ─────────────────────────────────────────────────────────── */}
        <div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'var(--foreground)' }}>分析页</h1>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--muted-foreground)' }}>今日业务数据概览与访客趋势</p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'stretch' }}>
          ...
          <div style={{ ...cardStyle, flex: '6 1 0', minWidth: 0, gap: 22 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--foreground)', letterSpacing: '-0.2px' }}>今日销售</div>
                <div style={{ fontSize: 12, color: 'var(--muted-foreground)', marginTop: 3, fontWeight: 500 }}>销售总结</div>
              </div>
              <button
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--muted-foreground)', fontWeight: 500 }}>实际销售额</div>
                    <div style={{ fontSize: 9, color: 'var(--muted-foreground)', marginTop: 1 }}>本周累计</div>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--muted-foreground)', fontWeight: 500 }}>目标销售额</div>
                    <div style={{ fontSize: 9, color: 'var(--muted-foreground)', marginTop: 1 }}>本周目标</div>
                  </div>
                </div>
                <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--foreground)', letterSpacing: '-0.5px', fontVariantNumeric: 'tabular-nums' }}>

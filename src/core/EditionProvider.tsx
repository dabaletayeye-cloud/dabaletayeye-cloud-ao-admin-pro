...
export function EditionProvider({ children }: { children: ReactNode }) {
  const [config, setConfig] = useState<EditionConfig>(DEFAULT_EDITION);
  const [loading, setLoading] = useState(true);
  const refreshEdition = useCallback(async () => {
    try {
      const next = await apiAdapter.getSystemEdition();
      if (next?.edition) setConfig({ ...next, enabledModules: Array.isArray(next.enabledModules) ? next.enabledModules : [] });

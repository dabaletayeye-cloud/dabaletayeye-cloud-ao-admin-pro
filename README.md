...
顶部语言菜单和“系统设置 → 默认语言”支持以下语言，并会保存到浏览器本地：
...
切换后会同步更新页面语言标识、顶部常用操作、登录页语言菜单、侧边栏核心导航、面包屑、多标签页、工作台、分析页和组件中心的界面文案。项目使用 `i18next` + `react-i18next`，语言资源按“语言/模块”拆分并以 JSON 存储，例如 `src/i18n/locales/zh-CN/components.json` 和 `src/i18n/locales/en-US/dashboard.json`；新增或迁移页面时请在对应模块 JSON 文件增加词条，并在组件中使用 `useTranslation()` 的 `t()` 获取文案。组件示例还提供共享文本兜底翻译，避免主题切换后遗漏硬编码控件标签。尚未加入词条的内容会按 i18next 的回退语言显示。
